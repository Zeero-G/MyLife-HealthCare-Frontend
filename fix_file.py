...
# strip everything from the first `{selectedMember ? (` to the end of the file
start_idx = text.find('{selectedMember ? (')
if start_idx != -1:
    new_text = text[:start_idx] + correct_block
    with open('/home/malan/Desktop/ZeeroG/MyLife-Frontend/src/components/FamilyDashboard.tsx', 'w') as f:
        f.write(new_text)
    print("Fixed file syntax")
else:
    print("Failed to fix")
